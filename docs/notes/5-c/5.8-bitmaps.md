...
You will often see this shown as `4fb07c` or `#4fb07c`. The red, green, and blue color components are each represented with two hexadecimal digits.
...
- Each row stores the color components for each pixel in that row.
  ...
  - The next three elements in the array are the color components for the second pixel.
  - And so on.
...
Include the `stdint.h` header to get access to the `uint8_t` data type. This type is guaranteed to be an 8-bit unsigned integer.
...
for (int row = 0; row < BYTE_ROWS; row++) {
    ...
    // Loop through all bytes for this row of pixels
