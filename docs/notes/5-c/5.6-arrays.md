...
- The size of each element is determined by the array variable's data type. In this example, an `int` is used, so each element uses four bytes.
...
You may have to use a `#define` instead of a regular constant:

```c
#define NUM_SCORES 5
int scores[NUM_SCORES] = { 1, 2, 3, 4, 5 }
